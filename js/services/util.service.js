...
// Set query string params and url
function setQueryStringParams(bookId = null) {
  const filterBy = getFilterBy()

  const queryStringParams = `?price=${filterBy.price}&txt=${
    filterBy.txt
  }&rate=${filterBy.rate}${bookId ? `&id=${bookId}` : ''}`
  const newUrl =
